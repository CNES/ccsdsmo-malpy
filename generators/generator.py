...
class MALTypeModuleGenerator(object):
    ...
    def save_services(self):
        ...
        service_string_list = ', '.join(["'{}'".format(s) for s in self.services.keys()])
    ...
    def save_datatypes(self):
        ...
        if self.datatypes[-2:] == '\n\n':
            self.datatypes = self.datatypes[:-2]
    ...
    "    \"\"\"All MAL errors.\"\"\"\n"
        ...
        for e in d:
            self.write_datatype(4*' ' + "{} = {}".format(e.name, e.number))
            if e.comment is not None:
                self.write_datatype(' # '+e.comment)
            self.write_datatype('\n')
    ...
    "    {name} = {nvalue}".format(name=item.value, nvalue=item.nvalue)
            ...
            if item.comment is not None:
                self.write_datatype(' # '+item.comment)
                self.write_datatype('\n')
        self.write_datatype("\n")
        self.write_datatype("\n")
    ...
    def write_serviceprovider_module(self, service):
        ...
        self.write_service(servicename,
    DO_NOT_EDIT +
    "\n" +
    "\"\"\"{}\"\"\"\n".format(service.comment) +
    "\n" +
    "{}\n".format("\n".join(IMPORTS[self.area.name])) +
    "from mo.{} import *\n".format(self.area.name.lower()) +
    "\n" +
    ...
    def generate(self):
        ...
        for area_node in list(root):
            ...
            for service in services:
                self.write_serviceprovider_module(service)
